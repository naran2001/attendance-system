...
(async () => {
  try {
    await pool.execute(
      "ALTER TABLE attendance ADD COLUMN status ENUM('pending', 'approved') DEFAULT 'approved'"
    );
  ...
  try {
    await pool.execute(
      "ALTER TABLE users ADD COLUMN shift_end TIME DEFAULT '17:00:00'"
    );
  } catch (e: any) {
    if (e.code !== "ER_DUP_FIELDNAME") {
      console.error("Migration error:", e);
  ...
  try {
    await pool.execute(
      "ALTER TABLE attendance ADD COLUMN early_checkin_minutes INT DEFAULT 0"
    );
